...
public class ContribuicaoController {
    ...
    public ContribuicaoController(ContribuicaoService contribuicaoService, UsuarioService usuarioService) {
    ...
    public ResponseEntity<Map<String, String>> addContribuicao(@RequestBody Map<String, String> formData) {
        ...
        if (!valorMonetario.isEmpty() && !moeda.isEmpty() && u.getId() != 0L ) {
            try {
                contribuicaoService.adicionar(new Contribuicao(null,Double.valueOf(valorMonetario),moeda,dataCriacao,u));
            } catch (ExcecaoP e) {
